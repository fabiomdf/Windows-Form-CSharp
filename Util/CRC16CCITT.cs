...
namespace Util
{
    public class CRC16CCITT
    {
        ...
        private ushort ByteCRC(ushort valor, ushort crc)
        {
            return (ushort)((crc << 8) ^ crcCCITT[((crc >> 8) ^ valor) & 0xFF]);
        }

        public static ushort Calcular(byte[] buffer)
        {
            return Calcular(buffer, 0, buffer.Length);
        }
        ...
        public static ushort Calcular(string buffer)
        {
            // Converte a string para um array de bytes
            UTF8Encoding encoder = new UTF8Encoding();
            byte[] byteBuffer = encoder.GetBytes(buffer);

            // Chama o método sobrecarregado
            return Calcular(byteBuffer);
        }

        public static byte[] ValidarBytes(byte[] bytes)
        {
            ushort crc = Calcular(bytes);
            return BitConverter.GetBytes(crc);
        }
        public static void AtualizarCRC(string nomeArquivo, int crcPosition)
        {
            int crcPosition2 = crcPosition + 1;
            // Ler os dados.
            ...
            ushort crc16 = CalcularCRC(dados, crcPosition);

            BitConverter.GetBytes(crc16).CopyTo(dados, crcPosition);

            // Escrever os dados com o CRC atualizado.
            fs = File.Create(nomeArquivo);
        ...
        public static bool ValidarCRC(string nomeArquivo, int crcPosition)
        {
            int crcPosition2 = crcPosition + 1;
            ...
            return ((BitConverter.GetBytes(crc16)[0] == dados[crcPosition]) && (BitConverter.GetBytes(crc16)[1] == dados[crcPosition2]));
        }
    }
}
